...
phrases = [phrase.split(' ') for phrase in input.split('\n')]

valid_phrases = [phrase for phrase in phrases if len(phrase)==len(np.unique(phrase))]
...
valid_phrases = [phrase for phrase in phrases if len(phrase)==len(np.unique([''.join(sorted(word)) for word in phrase]))]
